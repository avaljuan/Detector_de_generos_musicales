...
calcular_tempo <- function(ruta_archivo) {
  # Importamos librosa dentro de R


  # 1. Cargar el audio
  audio_data <- librosa$load(ruta_archivo)
  y <- audio_data[[1]]
  sr <- audio_data[[2]]

  # 2. Calcular el "Onset Strength Envelope" (la curva de inicio de notas)
  oenv <- librosa$onset$onset_strength(y=y, sr=sr)

  # Esto estima el tempo predominante (un solo número)
  tempo_estimado <- librosa$beat$tempo(onset_envelope=oenv, sr=sr)
  print(paste("El tempo detectado es:", tempo_estimado))

  return(tempo_estimado)
}

calcular_tempo("..\\MusicaRap\\El Jincho - Que Esta Pasando (VIDEOCLIP OFICIAL).mp3")
...
for (i in 1:length(nombres_canciones)){
  canciones[[i]] <- data.frame(nombre_cancion = nombres_canciones[i], tempo=calcular_tempo(paste0("..\\MusicaRap\\",nombres_canciones[i])),genero="Rap")
