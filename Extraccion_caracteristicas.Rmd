...
#Si no se tiene el entorno conda instalado
```{r}
...
#Calculamos las características realtivas a la librería librosa
```{r}
...
#Función para importar audio normalizado (para las características de las librerías de R)
```{r}
...
importar_audio_normalizado <- function(path) {
  ...
  # Recortar para tener 90s
  w <- tuneR::extractWave(w, from = 0, to = 90, xunit = "time")
  
  # Extraer vector para procesar
  x  <- w@left
  fs <- w@samp.rate
  
  # Quitar DC (centrar la onda en 0)
  x <- x - mean(x)
  
  # Construir objeto Wave para uso interno en R
...
#Función características espectrales
...
#Generamos los csv con todas las características por géneros 
```{r}
for (g in generos) {
