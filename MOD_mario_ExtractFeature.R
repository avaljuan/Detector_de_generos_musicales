...
#Espectrograma MEL v2
espectro_MEL_v2 <- function( audio, FS_OBJETIVO ){
  ...
  # Calculamos la matriz MFCC (filas=tiempo, columnas=13 coeficientes)
  mel_spect_db <- melfcc(audio, 
                         sr = FS_OBJETIVO, 
                         numcep = 13, 
                         wintime = 0.025, 
  ...
  # Calculamos la media de CADA coeficiente (del 1 al 13)
  mel_means <- colMeans(mel_spect_db, na.rm = TRUE)
  
  # Calculamos la desviación estándar de CADA coeficiente (del 1 al 13)
  mel_sds <- apply(mel_spect_db, 2, sd, na.rm = TRUE)
  
  # Devolvemos un vector largo: primero las 13 medias, luego las 13 sds
  return(c(mel_means, mel_sds))
}
...
#MEL 26 caracteristicas
matriz_mel <- matrix(nrow = length(songs_list), ncol = 26)
...
for (i in 1:length(songs_list)) {
  ...
  #  MEL v2 
  # Guardamos los 26 valores directamente en la fila 'i' de la matriz
  matriz_mel[i, ] <- espectro_MEL(songs_list[[i]], songs_list[[i]]@samp.rate)
}
...
#Añadimos 
# Preparamos los nombres de las columnas MEL: MEL_1_mean, MEL_2_mean...
nombres_mel_mean <- paste0("MEL_", 1:13, "_mean")
nombres_mel_sd   <- paste0("MEL_", 1:13, "_sd")
nombres_mel_total <- c(nombres_mel_mean, nombres_mel_sd)

# Convertimos la matriz MEL a dataframe y le ponemos nombres
df_mel <- as.data.frame(matriz_mel)
colnames(df_mel) <- nombres_mel_total

# Unimos todo (bind_cols es de dplyr, si no tienes dplyr cargado usa cbind)
features <- cbind(features, df_mel)
...
write.csv(features, "features_prueba_jazz.csv", row.names = FALSE)
