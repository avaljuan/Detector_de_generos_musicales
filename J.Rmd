...
genero <- "Ópera" #pon aqui el genere que te ha tocado, primera en mayus
carpeta <- "cancionwav" #pon aqui el nombre de la carpeta que contiene los archivo
```
...
# ==========================
# MEL / MFCC
# 26 features: 13 medias + 13 sds (por coeficiente MFCC)
# ==========================
...
espectro_MEL_v2 <- function(audio, FS_OBJETIVO ){
  ...
  # Matriz MFCC: filas=tiempo, columnas=13 coeficientes
  ...
  # 13 medias y 13 sds (una por coeficiente)
...
for(p in paths) {
  songs_list <- append(songs_list,importar_audio_normalizado(path = p))
}
...
# MEL 26 características (13 mean + 13 sd)
...
for (i in 1:length(songs_list)) {
  ...
  matriz_mel[i, ] <- espectro_MEL_v2(songs_list[[i]], 48000)
...
# Nombres columnas MEL (13 mean + 13 sd)
...
features <- features %>% mutate(Genero = "Opera")

# Exportamos

write.csv(features, "features_opera.csv", row.names = FALSE)
```
